...
export class ProfilePage implements OnInit, OnDestroy {
  ...
  loadProfessionalData(professionalId: string): void {
    this.isLoading = true;
    this.error = '';
    ...
    this.professionalSub = this.professionalService.getById(Number(professionalId)).subscribe({
      ...
      error: (error) => {
        ...
        this.error = 'Failed to load professional data';
        this.isLoading = false;
      }
    });
  }
  ...
  loadProfessionalServices(professionalId: number): void {
    this.servicesSub = this.professionalService.getServices(professionalId).subscribe({
      next: (services) => {
        if (this.service) {
  ...
  onSaveClick(): void {
    ...
    this.professionalService.update(Number(this.service.id), professionalData).subscribe({
      ...
      error: (err: unknown) => {
        ...
        this.showToast = true;
        this.toastMessage = 'Error saving changes';
